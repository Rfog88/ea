...
kind: skill
name: daily-briefing
description: Assemble a 3-bullet morning briefing (open reminders + yesterday's notes + today's calendar) and SMS it to Ryan. Silent if there is nothing to report.
metadata:
  requires_env:
    - PAPERCLIP_API_URL
    - PAPERCLIP_API_TOKEN
  ...
  primary_users: [ea]
  storage: reads paperclip-issues + vault notes + graph calendar; sends via Twilio
  status: needs-paperclip-api-confirm
---

# daily-briefing

The 07:30 ET routine's worker. Pulls three sources, formats a compact SMS, sends to Ryan. Sends NOTHING if all three are empty — no "all clear" pings.
...
6. Else send via Twilio to `BOARD_PHONE`.
