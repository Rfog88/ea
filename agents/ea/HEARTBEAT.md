...
1. **Check Inbox for new Issues labeled `sms-inbound`.** For each:
   ...
   c. If you're confident in the response, call `send-sms` to the originator.
   d. If you're unsure, ask ONE clarifying question via `send-sms` — don't ping-pong.
...
3. **Daily briefing** runs via the `briefing-morning` routine (not heartbeat work). On your next tick after 07:30 ET, verify it completed and Ryan received the SMS. If not, run `diagnose-why-work-stopped` and escalate Tier 1 — `external-quota-exceeded` (A2P/Twilio) or `adapter-broken` (Graph), per the diagnosis.
